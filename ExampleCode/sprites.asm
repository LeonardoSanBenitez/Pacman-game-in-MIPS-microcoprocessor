...
exit_moviment:
	...
	move $s1, $a0
	move $s2, $a1
	jal return_wall #(Posi��o x do grid, posi��o Y do grid, *grid)
	bnez $v0, end_apply
	...
	la $s0, pacman
	sw $s3, 4($s0)	#grava a proxima posi��o para imprimir o sprite
	sw $s4, 8($s0)	#grava a proxima posi��o para imprimir o sprite
	#b apply_final
end_apply:
...
ret_score:
	la $a0, str #str � la do .data str: .asciiz "\nScore: "
	li $v0, 4
	syscall

	li $v0, 5

	la $s0, score #score � um .data score: .word 0
	lw $a0, 0($s0)
...
.ktext 0x80000180
  ...
  andi  $a0,$a0,0x007C
  add 	$a1, $a1, $a0 	# Endereço do elemento na jtable
...
case0:
   ...
   # syscall
    ...
    la $a2, 0xffff0000  #Carregando endere�o com as informa��es do teclado
...
# Declarar todas as variaveis da tabela de exce��o

invalid: 		.asciiz "Exce��o n�o encontrad a"
